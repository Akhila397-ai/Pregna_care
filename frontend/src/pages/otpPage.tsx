    ...
    <div className="min-h-screen flex flex-col lg:flex-row font-sans bg-[#f5f7f0]">
      ...
      <div className="flex flex-col justify-between px-8 py-10 lg:px-16 lg:py-14 w-full lg:w-1/2 xl:w-5/12">
        ...
        <div className="mb-8">
          ...
          <p className="text-[#2ecc71] font-semibold text-base mt-0.5 truncate max-w-xs">{email}</p>
        </div>
        ...
        <div className="flex gap-3 mb-6" onPaste={handlePaste}>
          {digits.map((d, i) => (
            <input
              ...
              className={`w-full aspect-square max-w-[56px] text-center text-2xl font-bold rounded-xl border-2 bg-white text-[#1a2e1a] outline-none transition-all duration-200
                ${d ? "border-[#2ecc71] bg-[#f0fdf4]" : "border-[#dde8dd]"}
        ...
        <button
          ...
          className="w-full py-3.5 rounded-xl bg-[#2ecc71] hover:bg-[#27b860] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-base shadow-lg shadow-green-200 transition-all duration-200 flex items-center justify-center gap-2 mb-4"
        >
          {loading ? (
            <>
        ...
        <p className="text-center text-sm text-[#5a7a5a]">
          ...
          {resendTimer > 0 ? (
            <span className="text-[#8fba8f]">
              Resend in <span className="font-semibold text-[#2ecc71]">{resendTimer}s</span>
            </span>
          ) : (
            <button onClick={handleResend} className="text-[#2ecc71] font-semibold hover:underline">
              Resend Code
            </button>
          )}
        </p>
          ...
          <div className="flex items-center gap-3">
            {["Create Account", "Verify Email", "You're In!"].map((label, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className={`text-xs font-medium ${i === 1 ? "text-white" : "text-white/50"}`}>{label}</span>
                {i < 2 && <div className={`h-px w-6 ${i < 1 ? "bg-[#2ecc71]" : "bg-white/20"}`} />}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="lg:hidden relative h-44 w-full overflow-hidden">
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url('https://images.unsplash.com/photo-1609220136736-443140cffec6?w=800&q=80')` }}
        />
