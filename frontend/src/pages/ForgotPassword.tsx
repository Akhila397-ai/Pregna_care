...
import { forgotPassword,verifyForgotOtp,resetPassword } from "../api/auth.api";
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col lg:flex-row font-sans bg-[#f5f7f0]">
      ...
      <div className="flex flex-col px-8 py-6 lg:px-16 lg:py-10 w-full lg:w-1/2 xl:w-5/12 h-full overflow-y-auto">
        ...
        {step !== "success" && (
          <button
            onClick={() => step === "email" ? navigate("/login") : setStep(steps[currentIdx - 1] as Step)}
        ...
        {step === "email" && (
          <div className="space-y-5 flex-1">
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-[#2ecc71] hover:bg-[#27b860] active:scale-[0.98] text-white font-bold text-base shadow-lg shadow-green-200 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
            >
        ...
        {step === "otp" && (
          <div className="space-y-6 flex-1">
            {error && <p className="text-red-500 text-sm">{error}</p>}
            <div>
              <label className="block text-sm font-semibold text-[#2d4a2d] mb-4">Enter 6-digit code</label>
              <div className="flex gap-2 sm:gap-3">
                {otp.map((digit, idx) => (
                  <input
                    ...
                    onBlur={() => setFocused(null)}
                    className={`w-11 h-13 sm:w-12 sm:h-14 text-center text-lg font-bold rounded-xl border-2 bg-white text-[#1a2e1a] outline-none transition-all duration-200 ${
                      focused === `otp-${idx}`
                        ? "border-[#2ecc71] shadow-[0_0_0_3px_rgba(46,204,113,0.15)]"
                        : digit
        ...
        {step === "reset" && (
          <div className="space-y-5 flex-1">
            ...
            {/* New password */}
            <div>
              <label className="block text-sm font-semibold text-[#2d4a2d] mb-1.5">New Password</label>
              <div className="relative">
                <input
            ...
            {/* Confirm password */}
            <div>
              <label className="block text-sm font-semibold text-[#2d4a2d] mb-1.5">Confirm Password</label>
              <div className="relative">
                <input
        ...
        {step === "success" && (
          <div className="flex-1 flex flex-col items-start gap-6">
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-[#2ecc71] hover:bg-[#27b860] active:scale-[0.98] text-white font-bold text-base shadow-lg shadow-green-200 transition-all duration-200"
            >
              Back to Login
            </button>
          </div>
