...
import { IUserMappedData, IDoctorMappedData } from '@/features/admin/types/admin.types';
...
const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(verifyDoctorThunk.fulfilled, (state, action) => {
    ...
    // ── Unblock User ───────────────────────────
    builder.addCase(unblockUserThunk.fulfilled, (state, action) => {
      const user = state.users.find((u) => u._id === action.payload);
    ...
    // ── Approve Doctor ─────────────────────────
    builder.addCase(approveDoctorThunk.fulfilled, (state, action) => {
      const doc = state.doctors.find((d) => d._id === action.payload);
      if (doc) doc.status = 'approved';
    });

    // ── Reject Doctor ──────────────────────────
    builder.addCase(rejectDoctorThunk.fulfilled, (state, action) => {
      const doc = state.doctors.find((d) => d._id === action.payload);
      if (doc) doc.status = 'rejected';
    });

    // ── Block Doctor ───────────────────────────
    builder.addCase(blockDoctorThunk.fulfilled, (state, action) => {
      const doc = state.doctors.find((d) => d._id === action.payload);
      if (doc) doc.isBlocked = true;
    });

    // ── Unblock Doctor ─────────────────────────
    builder.addCase(unblockDoctorThunk.fulfilled, (state, action) => {
      const doc = state.doctors.find((d) => d._id === action.payload);
      if (doc) doc.isBlocked = false;
    });

    // ── Delete Doctor ──────────────────────────
    builder.addCase(deleteDoctorThunk.fulfilled, (state, action) => {
